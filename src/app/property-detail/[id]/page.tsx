      ...
      <main className="mx-auto min-h-[70vh] max-w-6xl px-4 py-8 sm:px-6">
        ...
        <div className="mt-4 grid gap-8 lg:grid-cols-[1.4fr_1fr]">
          ...
          <div>
            ...
            {listing.videoUrl ? (
              <div className="mt-4 overflow-hidden rounded-2xl border border-ink-950/10 shadow-card">
                <video
                  src={listing.videoUrl}
                  controls
                  playsInline
                  className="h-auto w-full bg-ink-950"
                />
              </div>
            ) : null}
            ...
            {listing.gallery.length ? (
              <div className="mt-4 grid grid-cols-3 gap-3 sm:grid-cols-4">
                {listing.gallery.map((g) => (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img
                    key={g}
                    src={g}
                    alt={listing.title}
                    className="h-24 w-full rounded-xl border border-ink-950/10 object-cover"
                  />
                ))}
              </div>
            ) : null}
